...
def encode(message, em_len, ps=None, hash_class=hashlib.sha1):
    #em_length = length of output we want
    halgo = hash_class(message)
    h = halgo.digest() #sha1 hash digest of message (len is 20 bytes)
    try:
        t = DIGEST_INFO_PREFIXES[hash_class] + h #t is the prefix for the hash + hash digest
    except KeyError:
        raise NotImplementedError('hash algorithm is unsupported', hash_class)
    if em_len < len(t) + 11:
        raise exceptions.MessageTooShort
    ps_len = em_len - len(t) - 3 # in out case its 256-35-3=218
    ps = b'\xff' * ps_len  
