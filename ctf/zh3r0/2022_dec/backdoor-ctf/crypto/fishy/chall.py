...
while len(pt) % 64 != 0:
    pt = "0" + pt
pt = hex(int(pt, 2))[2:].zfill(len(pt) // 16) ## this will create no difference mostly
ct = ""
for i in range(len(pt) // 16):
    xl = pt[16 * i : 16 * i + 8] # 4 hex digits, len(xl) = 8
    xr = pt[16 * i + 8 : 16 * i + 16] # 4 hex digits, len(xr) = 8
    # rounds
    for j in range(16):
        tmp = xl
        xl = bin(int(xl, 16) ^ int(processed_sub_keys[j], 16))[2:].zfill(32) # 32 binary digits
        xa = int(xl[:8], 2) # 8 binary digits, 1 int value btw 0 to 255
        xb = int(xl[8:16], 2) # 8 binary digits, 1 int value btw 0 to 255
        xc = int(xl[16:24], 2) # 8 binary digits, 1 int value btw 0 to 255
        xd = int(xl[24:32], 2) # 8 binary digits, 1 int value btw 0 to 255
