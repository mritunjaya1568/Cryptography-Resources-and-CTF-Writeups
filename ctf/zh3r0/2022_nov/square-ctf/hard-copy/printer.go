...
func initTLS() error {
	...
	var bigOne = big.NewInt(1) // bigOne = 1
	var bigTwo = big.NewInt(2) // bigTwo = 2

	p, err := rand.Prime(rand.Reader, bits/2) // p -> 1024 bit prime
	if err != nil {
		return fmt.Errorf("failed to get prime: %w", err)
	}

	q := new(big.Int) // 
	q.Xor(p, new(big.Int).Lsh(bigOne, bits/2-3)) // ensure q is not close to p //  q  = xor(p, 1 << 1021)
	for {
		if q.ProbablyPrime(20) { // miller rabin with n = 20
			break
		}
		switch q.Cmp(p) { // q - p
		case -1: // q < p
			q.Sub(q, bigTwo) // q = q - 2
		case 1: // q > p
			q.Add(q, bigTwo) // q = q + 2
